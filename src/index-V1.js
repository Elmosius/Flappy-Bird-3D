...
function createPipe(x, z, height, gap) {
  ...
  bottomPipeGeometry.setAttribute("uv2", new THREE.BufferAttribute(bottomPipeGeometry.attributes.uv.array, 2));
  ...
  topPipeGeometry.setAttribute("uv2", new THREE.BufferAttribute(topPipeGeometry.attributes.uv.array, 2));
  const topPipe = new THREE.Mesh(topPipeGeometry, barkMaterial);
  topPipe.position.set(x, randomGapY + gap / 2 + height / 2, z);
  scene.add(topPipe);

  // Bottom edge
  const bottomEdge = new THREE.Mesh(new THREE.CylinderGeometry(1.5, 1.5, 2, 32), barkMaterial);
  bottomEdge.position.set(x, randomGapY - gap / 2, z);
  scene.add(bottomEdge);

  // Top edge
  const topEdge = new THREE.Mesh(new THREE.CylinderGeometry(1.5, 1.5, 2, 32), barkMaterial);
...
scene.background = skyboxLoader.load(["./assets/skyBox/cube_right.png", "./assets/skyBox/cube_left.png", "./assets/skyBox/cube_up.png", "./assets/skyBox/cube_down.png", "./assets/skyBox/cube_back.png", "./assets/skyBox/cube_front.png"]);
...
function animate() {
  ...
  if (bird) {
    bird.position.x += 0.1;
    camera.position.set(bird.position.x - 5, bird.position.y + 2, bird.position.z);
