...
export default class Floor {
  constructor(scene, physicsWorld, camera) {
    ...
    this.camera = camera; // Tambahkan referensi kamera untuk penggulung tekstur
    this.rockyTexture = null; // Simpan tekstur untuk pengulangan
    this.initFloor();
  }
  ...
  initFloor() {
    ...
    this.floor = floor; // Simpan referensi lantai untuk memperbarui tekstur
  }

  update() {
    if (this.rockyTexture && this.camera) {
      // Gulir tekstur sesuai posisi kamera
      this.rockyTexture.offset.x = this.camera.position.x * 0.01; // Sesuaikan faktor kecepatan
